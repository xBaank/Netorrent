...
{
    public static IEnumerable<Func<(string, BList)>> GetTestData()
    {
        ...
        yield return () =>
            ("li1ei2ei3ee", new List<IBencodingNode>() { new BInt(1), new BInt(2), new BInt(3) });
        ...
        yield return () =>
            ("ll4:spamee", new List<IBencodingNode>() { new BList([new BString("spam")]) });

        yield return () =>
            ("l0:4:datae", new List<IBencodingNode>() { new BString(""), new BString("data") });
